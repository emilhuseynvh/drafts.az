...
const Footer = () => {
  ...
  const list = t.raw('footer.learn.list')
    ...
    <footer className='mt-36 mb-5'>
      <div className='wrapper md:block hidden rounded-2xl bg-[#F7F7F1] pl-6 pr-4 pt-12 pb-7 relative'>
        <div className='w-full flex justify-between'>
          <div className='w-1/2'>
            <ul className='list-disc ml-4'>
              <li className='text-base font-normal'>{t('footer.title')}</li>
            </ul>
            <h2 className='text-[2.45rem] mt-6'>{t('footer.headline')}</h2>
            <div className='mt-9'>
              <Button bgColor='#111213'>
                ...
                <p className='group-hover:text-[#F7F7F1] text-white relative z-10'>{t('footer.button')}</p>
              </Button>
            </div>
          </div>
          <div className='w-1/2 pl-16 flex pr-27 justify-between'>
            <div>
              <p className='text-xs font-normal text-[#3D3D3D] mb-2'>{t('footer.learn.title')}</p>
              <ul>
                {list.map((item: any, index: number) => (
                  <li key={index} className='my-1 text-base font-normal text-primary-bg'>{item}</li>
                ))}
              </ul>
            </div>
            <div>
              <p className='text-xs font-normal text-[#3D3D3D] mb-4'>{t('footer.contact')}</p>
              <ul>
        ...
        <div className='flex gap-2 mt-10'>
          <p className='text-xs text-primary-bg font-normal border-r pr-3 border-r-primary-bg'>Privacy Policy</p>
        ...
        <div className='pl-8 bg-[#F7F7F1] relative pr-4 pt-5 pb-7 border-t-r-4 border-b-l-4'>
          ...
          <div className='ml-5'>
            <Button bgColor='#151515' width='60%'>
              <p className='relative z-10 text-white'>{t('footer.button')}</p>
              <svg className='relative z-10' width='25' height='24' viewBox='0 0 25 24' fill='none' xmlns='http://www.w3.org/2000/svg' >
          ...
          <div className='flex mt-12 justify-between mb-20'>
            <div>
              <p className='text-base font-semibold text-[#3D3D3D] mb-3.5'>{t('footer.learn.title')}</p>
              <ul>
                {list.map((item: any, index: number) => (
                  <li key={index} className='my-1 text-base font-normal text-primary-bg'>{item}</li>
                ))}
